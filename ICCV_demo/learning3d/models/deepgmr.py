...
def gmm_register(pi_s, mu_s, mu_t, sigma_t):
	...
	'''
	device=pi_s.device    
	c_s = pi_s.unsqueeze(1) @ mu_s.to(device)
	c_t = pi_s.unsqueeze(1) @ mu_t.to(device)
...
# 	V = V.cuda() if torch.cuda.is_available() else V
	...
	R = V @ S @ U.transpose(1, 2).to(device)
...
class PointNet(nn.Module):
	def __init__(self, use_rri, use_tnet=False, nearest_neighbors=20):
		...
		print('args',use_rri,use_tnet,nearest_neighbors)        
