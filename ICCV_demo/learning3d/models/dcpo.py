...
class SVDHead_matching(nn.Module):
    def __init__(self, emb_dims, input_shape="bnc"):
        super(SVDHead_matching, self).__init__()
...
#         src_centered = src - src.mean(dim=2, keepdim=True)
        src_centered = topk_src - topk_src.mean(dim=2, keepdim=True)
...
class DCP_matching(nn.Module):
	def __init__(self, feature_model=DGCNN(), cycle=False, pointer_='transformer', head='svd'):
		...
		if head == 'mlp':
			...
			raise Exception('Not implemented')
		self.head = SVDHead_matching(self.emb_nn.emb_dims)
