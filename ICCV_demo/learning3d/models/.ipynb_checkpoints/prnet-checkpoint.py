...
class GraphHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        return {'H':H,'P':src_centered,'Q':src_corr_centered}
...
class PRNet_mod(nn.Module):
    def __init__(self, emb_nn='dgcnn', attention='transformer', head='svd', emb_dims=512, num_keypoints=512, num_subsampled_points=768, num_iters=3, cycle_consistency_loss=0.1, feature_alignment_loss=0.1, discount_factor = 0.9, input_shape='bnc'):
        ...
        #### GraphHead ####
        self.graph_head= GraphHead(emb_dims=self.emb_dims, cat_sampler='softmax')
...
#         # get the last graph adjacency
#         signature=True
#         if signature:
#             srcO, tgtO, src_embedding, tgt_embedding, temperature, feature_disparity = self.predict_embedding(src, tgt)
            
#             self.mem['graph'] = self.graph_head(src_embedding, tgt_embedding, srcO, tgtO, temperature)
# #             self.graph=self.graph_head(src, tgt)
...
class PRNet(nn.Module):
    def __init__(self, emb_nn='dgcnn', attention='transformer', head='svd', emb_dims=512, num_keypoints=512, num_subsampled_points=768, num_iters=3, cycle_consistency_loss=0.1, feature_alignment_loss=0.1, discount_factor = 0.9, input_shape='bnc',n_heads=4):
        ...
        if attention == 'identity':
            ...
            self.attention = Transformer(emb_dims=self.emb_dims, n_blocks=1, dropout=0.0, ff_dims=1024, n_heads=self.n_heads)
        else:
            raise Exception("Not implemented")
